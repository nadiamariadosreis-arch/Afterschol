...
export function ChangePasswordForm() {
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4 max-w-sm">
      ...
      />
      {status === "done" ? <p className="text-teal-dark text-[15px]">Senha atualizada.</p> : null}
      {status === "error" ? (
        <p className="text-coral-dark text-[15px]">Não foi possível salvar. Tente novamente.</p>
      ) : null}
