...
import { setJogoCapaPathAction } from "./actions";

export function CapaUploadForm({ jogoId, hasExisting }: { jogoId: string; hasExisting: boolean }) {
  ...
  async function handleChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    const { error } = await supabase.storage.from("jogos-capas").upload(path, file, {
    ...
    await setJogoCapaPathAction(jogoId, path);
    setStatus("done");
  }
