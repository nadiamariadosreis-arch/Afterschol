...
export async function generateContentPieces(
  ...
  try {
    ...
    if (pillars.length === 0) {
      return { error: "Defina os pilares de conteúdo na etapa de identidade." };
    }
    ...
    const pieces = await generateJSON<GeneratedPiece[]>(
    ...
    return { error: `Não foi possível gerar pautas agora (${message}). Tente novamente.` };
  }
}
